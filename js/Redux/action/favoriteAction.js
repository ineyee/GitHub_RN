...
export function favoriteFetchData(flag) {
	return dispatch => {
		dispatch({type: Type.FAVORITE_FETCH_DATA});

		FavoriteDao.getAllItems(flag)
			.then(JsonData => {
				dispatch({
					type: Type.FAVORITE_FETCH_DATA_SUCCESS,
					data: JsonData,
					selectedType: flag
				});
			})
			.catch(error => {
				console.log('favoriteFetchData失败：', error);
				dispatch({type: Type.FAVORITE_FETCH_DATA_FAILURE});
			})
	}
}

// RefreshHeader
export function favoriteRefreshHeader(flag) {
	return dispatch => {
		dispatch({type: Type.FAVORITE_REFRESH_HEADER});

		FavoriteDao.getAllItems(flag)
			.then(JsonData => {
				dispatch({
					type: Type.FAVORITE_REFRESH_HEADER_SUCCESS,
					data: JsonData,
					selectedType: flag
				});
			})
			.catch(error => {
				console.log('favoriteFetchData失败：', error);
				dispatch({type: Type.FAVORITE_REFRESH_HEADER_FAILURE});
			})
	}
}
